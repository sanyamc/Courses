...
ClusterPlot <- function(observations=sampleObservations, centers=centersGuess, labels=labelsRandom)
{
  ...
  x_Min <- min(observations[,2], centers[,1], na.rm=T)
  x_Max <- max(observations[,2], centers[,1], na.rm=T)
  y_Min <- min(observations[,2], centers[,1], na.rm=T)
  y_Max <- max(observations[,2], centers[,1], na.rm=T)
