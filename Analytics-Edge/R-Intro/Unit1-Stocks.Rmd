...
png=read.csv('procterGamblestock.csv')
...
ggplot(aes(x=Date,y=StockPrice),data=coke)+geom_line(aes(color='Coke'))+geom_line(data=png,aes(color='PnG'))+geom_line(data=ibm,aes(color='IBM',colour='black'))+geom_line(data=boeing,aes(color='Boeing'))+geom_line(data=ge,aes(color='GE'))+scale_x_date(breaks="2 year")+geom_vline(xintercept=as.numeric(as.Date('2000-03-01')),alpha=1/4,linetype=2)

```
Lets observe the data withing 5 year period.
```{r}

ggplot(aes(x=Date,y=StockPrice),data=coke)+geom_line(data=png,aes(color='PnG'))+geom_line(data=ibm,aes(color='IBM',colour='black'))+geom_line(data=boeing,aes(color='Boeing'))+geom_line(data=ge,aes(color='GE'))+geom_line(aes(color='Coke'))+scale_x_date(limits=c(as.Date("2004-01-01"),as.Date("2005-12-01")))+geom_vline(xintercept=as.numeric(as.Date('2000-03-01')),alpha=1/4,linetype=2)+scale_y_discrete(breaks=seq(0,200,5))
